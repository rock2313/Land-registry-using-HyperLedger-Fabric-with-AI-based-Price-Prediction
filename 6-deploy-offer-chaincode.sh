...
docker exec cli peer lifecycle chaincode package ${CHAINCODE_NAME}.tar.gz \
  --path /opt/gopath/src/github.com/chaincode/offer-contract \
  --lang golang \
...
echo "Verify with: docker exec cli peer lifecycle chaincode querycommitted -C landregistry"